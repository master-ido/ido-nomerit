...
def euler_2nd_order(dv_dt, initial_t, initial_v, initial_y, final_t, h, t_results=None, y_results=None, v_results=None):
    ...
    return euler_2nd_order(dv_dt, initial_t + h, initial_v + h * dv_dt(initial_t, initial_v, initial_y), initial_y + h * initial_v, final_t, h, t_results, y_results, v_results)
...
def present_rk_4(dy_dx, initial_x, initial_y, final_x, h):
    x_vals, y_vals = runga_kutta_4th_order(dy_dx, initial_x, initial_y, final_x, h)
    plt.plot(x_vals, y_vals, marker='o', linestyle='-', color='blue', label="Euler's Method")
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title("Euler's Method Approximation")
...
def finite_diff_2nd_ODE(p_x, q_x, r_x, x_0, y_0, x_n, y_n, h):
    n = int((x_n - x_0) / h)
    mat = np.zeros([n - 1, n - 1])
    for i in range(n - 1):
        for j in range(n - 1):
            if i == j:
                mat[i][j] = -(2 + (h ** 2) * q_x(x_0 + (i + 1) * h))
            if i == (j - 1):
                mat[i][j] = (1 - (h / 2) * p_x(x_0 + (i + 1) * h))
            if (i - 1) == j:
                mat[i][j] = (1 + (h / 2) * p_x(x_0 + (i + 1) * h))
    res = [(h ** 2) * r_x(x_0 + (1 + i) * h) for i in range(n - 1)]
    res[0] -= y_0 * (1 + (h / 2) * p_x(x_0 + h))
    res[n - 2] -= y_n * (1 - (h / 2) * p_x(x_n - h))
    vec = banded_system(mat, res)
    return [x_0 + i * h for i in range(n + 1)], [y_0] + vec + [y_n]

def present_finite_diff(p_x, q_x, r_x, x_0, y_0, x_n, y_n, h):
    t_vals, y_vals = finite_diff_2nd_ODE(p_x, q_x, r_x, x_0, y_0, x_n, y_n, h)
    plt.plot(t_vals, y_vals, marker='o', linestyle='-', color='blue', label="finite_diff Method")
    plt.xlabel('t')
    plt.ylabel('y')
    plt.title("finite_diff Approximation")
    plt.grid(True)
    plt.legend()
    plt.show()

def shooting_with_RK4(dv_dt, initial_t, initial_y, final_t, final_y, h, tol=10**-5, first_guess=None, second_guess=None):
    if first_guess is None:
        first_guess = initial_y
        second_guess = initial_y + h
    t_first, v_first, y_first = runga_kutta_4_order_2nd(dv_dt, initial_t, first_guess, initial_y, final_t, h)
    t_second, v_second, y_second = runga_kutta_4_order_2nd(dv_dt, initial_t, second_guess, initial_y, final_t, h)
    first_error = y_first[-1] - final_y
    second_error = y_second[-1] - final_y
    next_guess = second_guess - second_error * ((second_guess - first_guess) / (second_error - first_error))
    t_next, v_next, y_next = runga_kutta_4_order_2nd(dv_dt, initial_t, next_guess, initial_y, final_t, h)
    if abs(y_next[-1] - final_y) < tol:
